...
const toLastNames = people => {
  let Arr = people.map(folks => folks.firstName + " " + folks.lastName)
  return Arr;
};
...
const addValues = (arr) => {
  let sum = arr.reduce((accumulator, values) => accumulator += values, 0);
    return sum
};
...
const addPurchases = (arr) => {
  let sum = arr.reduce((accumulator, values) => accumulator += values.purchasePrice, 0);
  return sum
};
...
const countNumberOfElements = (arr) => {
  let summ = arr.reduce((accumulator, values) => values ? accumulator +1:accumulator+0, 0);         
  return summ;
};
