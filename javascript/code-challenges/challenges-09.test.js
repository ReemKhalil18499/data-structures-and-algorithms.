...
const maxInArray=(arr)=>{
  let max =arr.reduce((acc,value)=>Math.max(acc, value),0);
  return max;
};
...
const updateNumbers = (obj) => {
  ...
  let Arr2 = Arr.map(keyValue=>keyValue.join(": "));
  return Arr2;
};
...
const getHouses = (arr) => {
  let house1 = [];
  arr.forEach((character) => { house1.push(character.house); });
  return house1;
};
