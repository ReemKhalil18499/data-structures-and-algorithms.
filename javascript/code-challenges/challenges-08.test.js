...
const isCapitalized = (str) => {
  const regular = /(\b[A-Z].*?\b(.)??)/g;
  let regular2 = str.match(regular);
  if (regular2) {
    return regular2;
  } else {
    return [];
...
const citiesAtoJ = (arr) => {
  ...
  for(let i=0;i<arr.length;i++){
    let regular =/^([A-J][a-zA-Z]+)$/gm.test(arr[i]);
    if(regular){
      Arr.push(arr[i]);
    }
  }
